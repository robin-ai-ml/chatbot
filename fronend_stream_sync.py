...
# Define the async chat function to interact with the backend
async def chat_fn(message, chatbot):
    print(f"Message to backend: {message}, {get_current_time()}")  # Debugging output
    try:
        async with httpx.AsyncClient(timeout=None) as client:
            async with client.stream("GET", "http://localhost:8000/chat", json={"message": message, "history": []}) as response:
                full_response = ""
